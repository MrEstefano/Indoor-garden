...
/**************************************************************************************/
/*  Name    : Indoor Garden V35 Less Class functions more Local variables/ functions            */
...
typedef struct encoderButton {volatile bool middle = false; volatile bool lastMiddleState = false; int16_t last; int16_t value; };encoderButton Rotary_pointer;
...
void setup() {
  ...
  Timer1.initialize(500);   //Rotary Encoder is triggered by Timer 1 overflow period 50ms
...
void checkRotaryEncoder(){
  ...
  if (buttonState == ClickEncoder::Clicked ) {  
    pressButtonAction(); //manipulate variables
    flag = true;   //raise the flag to update the screen
  }
  
  //Check which direction the rotary is going
  Rotary->value += encoder->getValue();  
  value = Rotary->value>>1; //bittshift costs less computation then devide by 2
  if (value > Rotary->last) {  
    Rotary->last = value;
    navigateInMenuDown();  //manipulate variables
    flag = true;  //raise the flag to update the screen
  }
  else if (value < Rotary->last) {
    Rotary->last = value;
    navigateInMenuUp();  //manipulate variables
    flag = true;  // /raise the flag to update the screen
  }     
}
...
void navigateInMenuUp(){  
  menu.upFlag = true; 
  if(menu._parent == MAIN_MENU){       
    if(menu._child > SETPOINT){
       menu._child = static_cast<int>(menu._child) -1; //cast enum type to integer in order to increment or decrement the count
    }
    else{
      menu._child = SETPOINT;
    }     
    frameAligment(); 
  }
  else if(menu._parent == SUB_MENU){    
...
void navigateInMenuDown(){  
  menu.downFlag = true; 
  if(menu._parent == MAIN_MENU){      
    if(menu._child < RESET){
        menu._child = static_cast<int>(menu._child) + 1;  //cast enum type to integer in order to increment or decrement the coun
    }
    else{
      menu._child = RESET;
    }
...
void pressButtonAction(){ 
  if (menu._parent == MAIN_MENU){    
    ...
    menu._parent = MAIN_MENU; 
    menu._lastParent = SUB_MENU;   // flag to be set for csreen refresh when entering one of main screen manus       
  }    
}

void frameAligment(){
  if(menu.upFlag){
    if(menu._child == INTEGRA && menu.frame ==4){
      menu.frame--;
    }       
    else if(menu._child == PROPORT  && menu.frame ==3  ){ 
      menu.frame--;
    }  
    else if (menu._child == FLOW &&  menu.frame == 2 ){ 
      menu.frame--;  
    }  
    menu._lastChild = menu._child;
    menu.upFlag = false; 
  }

  if(menu.downFlag){
    if(menu._child == PROPORT && menu._lastChild == FLOW){
      menu.frame ++;
    }
    else  if(menu._child == INTEGRA && menu._lastChild == PROPORT){
      menu.frame ++;
    }
    else  if(menu._child == PUMP && menu._lastChild == INTEGRA && menu.frame!=4)  {
      menu.frame ++;
    }      
    menu._lastChild = menu._child;
    menu.downFlag = false;
...
void valueAdjustment(){
  if(menu.upFlag){  
    ...
    else if ( menu._child == FLOW ) {
     Water->supplyLimit  += 0.1 ;
    }
    else if ( menu._child == PROPORT  ) {
      PID->kp += 0.1;
    }
      else if ( menu._child == INTEGRA  ) {
      PID->ki += 0.1;      
    }
    menu.upFlag = false;
  } 
