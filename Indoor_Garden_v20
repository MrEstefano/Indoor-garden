...
void setup() {
  ...
  Timer1.initialize(2000);   //Rotary Encoder is triggered by Timer 1 overflow period
 ...
 const int numItems = 3;  // Number of menu items
  ...
  Serial.print("base: "); 
   ...
   for (int i = 0 + base; i < numItems + base; i++) {
            ...
            //bool highlight = (menuitem == i - base);  // Check if the current menu item is selected
            bool highlight = (menuitem == i+base) ? true : false;

            
            Serial.print("highlight: ");
            Serial.println(highlight);
            displayMenuItem(menuItems[i], positions[i - base], highlight);
            delay(15);
          }
...
void frameAligmentUP(){
  ...
  if (menuitem == PROPORT && lastMenuItem == INTEGRA){
    frame--;
  }  
  else if(menuitem == INTEGRA && lastMenuItem == PUMP){
    frame--;
  }  
  else if(menuitem == PUMP && lastMenuItem == RESET){
    frame--;
  }  
  ...
  if (frame < 1){
    ...
    lastMenuItem = menuitem;
       menuitem = static_cast<Menu_item>(static_cast<int>(menuitem) - 1);
      //menuitem++;  
      if (menuitem < SETPOINT){      
        menuitem = static_cast<Menu_item>(static_cast<int>(menuitem) + 1);
       // menuitem--;
...
void frameAligmentDown(){
  if(menuitem==PROPORT && lastMenuItem == FLOW){
      ...
      else  if(menuitem == PUMP && lastMenuItem == INTEGRA  ) {
        frame ++;
      }      
