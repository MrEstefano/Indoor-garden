...
/*            in order to adjust the treashold value                                  */
/**************************************************************************************/
...
void readEncoderButton(){
  if (digitalRead(ENCODER_SW) == LOW) {
    if (!isEditingSetpoint) {
      isEditingSetpoint = true; // Enter editing mode
      tempSetpoint = setpoint; // Use current setpoint as starting point
      delay(200); // Debounce delay
    } 
    else {
      Serial.println("button was pressed ");
      isEditingSetpoint = false; // Confirm the new setpoint
      setpoint = tempSetpoint; // Apply the new setpoint
      delay(200); // Debounce delay
    }
  }

  // If in editing mode, handle encoder rotation to adjust setpoint
  if (isEditingSetpoint) {
    ...
    // Keep the setpoint within valid range
    if (tempSetpoint < 0) tempSetpoint = 0;
    if (tempSetpoint > 100) tempSetpoint = 100; // Assuming 10-bit ADC for soil moisture
  }
}  
...
void updateDisplay() {
  if (isCountingDown) {    
    ...
    tft.println("SET:");
    tft.setCursor(75, 30);
    if (isFlashing && isEditingSetpoint) {
      tft.setTextColor(ST7735_RED); // Flashing text color red when editing
      tft.println(isEditingSetpoint ? tempSetpoint : setpoint);
    }
    else{
