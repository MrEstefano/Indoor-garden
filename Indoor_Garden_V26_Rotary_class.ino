...
private:
    ...
    bool middle;
    bool lastMiddleState;
    bool lastTimeChecked;
    int16_t last;
    int16_t value;

public:
  //constructor aka initialize  
  Rotary() :  navigateUpFlag(false),
    ...
    bool navUpFlag(){              
     return navigateUpFlag;        
    }

    bool navDownFlag(){              
     return navigateDownFlag;        
    }
    ...
    bool lastMiddleStateFlag(){              
     return lastMiddleState;        
    }


    uint16_t lastTimeChecked(){
...
void setup() {
  ...
  myRotary.lastValue() = encoder->getValue();
...
void loop() {  
  ...
  if (myRotary.navDownFlag() == true) {
    navigateInMenuDown();
    myRotary.navDownFlag() = false;
  }
  if (myRotary.navUpFlag() == true) {
    navigateInMenuUp();
    myRotary.navUpFlag() = false;
  }
...
void timerIsr() {
  ...
  if (buttonState == ClickEncoder::Clicked && !myRotary.lastTimeChecked()) {
    myRotary.middleFlag() = true;
    pressButtonAction();
  }
  myRotary.lastTimeChecked() = ( buttonState == ClickEncoder::Clicked); 

  //Check which direction the rotary is going
  myRotary.lastValue() += encoder->getValue();  
  if (myRotary.lastValue() > myRotary.lastValue()) {
    ROTARY.last = myRotary.lastValue();
    myRotary.navDownFlag() = true; 
  }
  else   if (myRotary.lastValue() < ROTARY.last) {
    ROTARY.last = myRotary.lastValue();
    myRotary.navUpFlag() = true;
  }
